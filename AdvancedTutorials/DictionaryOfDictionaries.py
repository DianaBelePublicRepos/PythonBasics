...
#Use these functions to convert any string to the appropriate Python data type
...
#Create an empty directory of people
people = {}
...
with open('sample.csv', encoding="utf-8", newline='') as f:
    ...
    f.readline()
    #Loop through the remaining rows one at a time, i is counter, row is entire row
    for i, row in reader:
        #From each data row in the CSV file create a Person object with a unique id and appropriate data types, add to people list.
...
#when above loop finishes show all objects in the people list
